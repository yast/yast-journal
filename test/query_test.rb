...
describe SystemdJournal::Query do
  ...
  describe "#entries" do
    it "returns a filtered set of entries" do
      ...
      expect(query).to receive(:journalctl_args).and_return("args")
      expect(SystemdJournal::Entry).to receive(:all).
        with("args").and_return("entries")
      expect(query.entries).to eq("entries")
    end
  end

  describe "#journalctl_args" do
    subject { SystemdJournal::Query.new(args).journalctl_args }
    ...
    context "when several filters and an interval are used" do
      ...
      it "combines them all" do
        expect(subject).
          to eq("--boot=\"-1\" --priority=\"3\" --unit=\"sshd.service\" \"/dev/sda\" \"/sbin/arp\"")
      end
    end
  end
end
